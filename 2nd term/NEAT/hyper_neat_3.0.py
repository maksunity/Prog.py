...
IMAGE_SIZE = (64, 64)  # Размер входного изображения для СУБСТРАТА
...
SUBSTRATE_HIDDEN_LAYER_SIZE = (10, 10) # <--- НОВЫЙ ПАРАМЕТР: Размер скрытого слоя (например, 10x10)
SUBSTRATE_HIDDEN_NODES = SUBSTRATE_HIDDEN_LAYER_SIZE[0] * SUBSTRATE_HIDDEN_LAYER_SIZE[1] # <--- НОВЫЙ ПАРАМЕТР
SUBSTRATE_OUTPUT_NODES = 10
...
NUM_GENERATIONS = 100 # Может потребоваться больше для HyperNEAT
OUTPUT_DIR = 'hyperneat2.0_output_mnist' # <--- ИЗМЕНЕНО: Папка вывода
...
def get_substrate_coords():
    input_coords = []
    y_input = -0.5 # Слой входов на y = -0.5
    for r in range(IMAGE_SIZE[0]):
        y_node = (r / (IMAGE_SIZE[0] - 1)) * 1.0 - 0.5 if IMAGE_SIZE[0] > 1 else 0.0 # Нормализуем от -0.5 до 0.5
        for c in range(IMAGE_SIZE[1]):
            x_node = (c / (IMAGE_SIZE[1] - 1)) * 1.0 - 0.5 if IMAGE_SIZE[1] > 1 else 0.0 # Нормализуем от -0.5 до 0.5
            input_coords.append((x_node, y_node + y_input)) # Сдвигаем слой

    hidden_coords = []
    y_hidden = 0.0 # Слой скрытых нейронов на y = 0.0
    for r in range(SUBSTRATE_HIDDEN_LAYER_SIZE[0]):
        y_node = (r / (SUBSTRATE_HIDDEN_LAYER_SIZE[0] - 1)) * 1.0 - 0.5 if SUBSTRATE_HIDDEN_LAYER_SIZE[0] > 1 else 0.0
        for c in range(SUBSTRATE_HIDDEN_LAYER_SIZE[1]):
            x_node = (c / (SUBSTRATE_HIDDEN_LAYER_SIZE[1] - 1)) * 1.0 - 0.5 if SUBSTRATE_HIDDEN_LAYER_SIZE[1] > 1 else 0.0
            hidden_coords.append((x_node, y_node + y_hidden)) # Сдвигаем слой


    output_coords = []
    y_output = 0.5 # Слой выходов на y = 0.5
    for i in range(SUBSTRATE_OUTPUT_NODES):
        x_node = (i / (SUBSTRATE_OUTPUT_NODES - 1)) * 1.0 - 0.5 if SUBSTRATE_OUTPUT_NODES > 1 else 0.0
        # output_coords.append((x_node, y_node + y_output)) # Сдвигаем слой
        output_coords.append((x_node, y_output))  # Сдвигаем слой

    return input_coords, hidden_coords, output_coords

SUBSTRATE_INPUT_COORDS, SUBSTRATE_HIDDEN_COORDS, SUBSTRATE_OUTPUT_COORDS = get_substrate_coords()

def generate_layer_weights(cppn, coords_layer1, coords_layer2): # <--- ИЗМЕНЕНО
    """
    Генерирует матрицу весов между двумя слоями субстрата, используя CPPN.
    """
    num_nodes_layer1 = len(coords_layer1)
    num_nodes_layer2 = len(coords_layer2)
    weights = np.zeros((num_nodes_layer1, num_nodes_layer2))

    for i, (x1, y1) in enumerate(coords_layer1):
        for j, (x2, y2) in enumerate(coords_layer2):
            cppn_input = [x1, y1, x2, y2, 1.0] # bias = 1.0
...
def activate_substrate(weights_ih, weights_ho, inputs_to_substrate): # <--- ИЗМЕНЕНО
    """
    Активирует субстрат с одним скрытым слоем.
    weights_ih: веса от входа к скрытому слою.
    weights_ho: веса от скрытого к выходному слою.
    inputs_to_substrate: плоский массив пикселей.
    """
    # Активация скрытого слоя
    hidden_inputs = np.dot(inputs_to_substrate, weights_ih)
    hidden_outputs = np.tanh(hidden_inputs) # Используем tanh для скрытого слоя

    # Активация выходного слоя
    final_inputs = np.dot(hidden_outputs, weights_ho)
    final_outputs = np.tanh(final_inputs) # Используем tanh и для выходного перед argmax
    return final_outputs



# --- Оценка генома CPPN ---
def eval_single_genome_hyperneat(genome, config):
    cppn = neat.nn.FeedForwardNetwork.create(genome, config)

    # Генерируем веса для каждого слоя соединений
    weights_ih = generate_layer_weights(cppn, SUBSTRATE_INPUT_COORDS, SUBSTRATE_HIDDEN_COORDS)
    weights_ho = generate_layer_weights(cppn, SUBSTRATE_HIDDEN_COORDS, SUBSTRATE_OUTPUT_COORDS)
    ...
    try:
        ...
        image_batch = data_holder.train_images[indices]
        true_labels_batch = data_holder.train_labels[indices]

        # Активируем субстрат на всем батче
        substrate_outputs_batch = activate_substrate(weights_ih, weights_ho, image_batch) # <--- ИЗМЕНЕНО
...
def post_training_analysis(cppn_genome, config, data_holder):
    print("\n--- Анализ после обучения на тестовых данных (HyperNEAT с скрытым слоем) ---")
    try:
        ...
        test_images_batch = data_holder.test_images # Весь тестовый набор как один батч
        test_labels = data_holder.test_labels

        raw_outputs_batch = activate_substrate(weights_ih, weights_ho, test_images_batch) # <--- ИЗМЕНЕНО
        preds = np.argmax(raw_outputs_batch, axis=1)
        print(f"Предсказания получены для {len(preds)} тестовых образцов.")

        raw_outputs_np = raw_outputs_batch # Уже массив NumPy

        accuracy = accuracy_score(test_labels, preds)
        print(f"Точность на тестовом наборе: {accuracy:.4f} ({int(accuracy * len(test_labels))}/{len(test_labels)})")
        # ... (остальная часть post_training_analysis остается без изменений,
        #      но она будет использовать preds и raw_outputs_np, полученные от субстрата с скрытым слоем)...
        # ... (код для Confusion Matrix, ROC, t-SNE) ...
...
def display_sample_predictions(cppn_genome, config, data_holder, total_images=10, images_per_plot=5):
    print(f"\n--- Отображение {total_images} случайных предсказаний (HyperNEAT с скрытым слоем) ---")
    try:
        cppn = neat.nn.FeedForwardNetwork.create(cppn_genome, config)
        weights_ih = generate_layer_weights(cppn, SUBSTRATE_INPUT_COORDS, SUBSTRATE_HIDDEN_COORDS)
        weights_ho = generate_layer_weights(cppn, SUBSTRATE_HIDDEN_COORDS, SUBSTRATE_OUTPUT_COORDS)
        ...
        test_labels = data_holder.test_labels
        # ... (остальная часть display_sample_predictions без изменений, но внутри цикла будет:)
        # substrate_output = activate_substrate(weights_ih, weights_ho, flat_image)
        ...
        for plot_idx in range(num_plots):
            ...
            for i, ax in enumerate(axes):
                ...
                substrate_output = activate_substrate(weights_ih, weights_ho, flat_image) # <--- ИЗМЕНЕНО
                prediction = np.argmax(substrate_output)
...
class DigitDrawerHyperNEAT: # <--- Новое имя класса
    def __init__(self, cppn_genome, config):
        self.cppn_genome = cppn_genome
        self.config = config
        self.cppn = neat.nn.FeedForwardNetwork.create(self.cppn_genome, self.config)
        # Генерируем веса для субстрата один раз при инициализации
        self.weights_ih = generate_layer_weights(self.cppn, SUBSTRATE_INPUT_COORDS, SUBSTRATE_HIDDEN_COORDS)
        self.weights_ho = generate_layer_weights(self.cppn, SUBSTRATE_HIDDEN_COORDS, SUBSTRATE_OUTPUT_COORDS)
        # ... (остальной __init__ без изменений) ...
        self.threshold = 0.3
        self.root = tk.Tk()
        self.root.title("Digit Recognizer (HyperNEAT with Hidden Layer)")
    ...
    def recognize(self):
        try:
            ...
            # Используем предсозданные веса
            substrate_output = activate_substrate(self.weights_ih, self.weights_ho,
                                                  inputs_to_substrate)  # <--- ИЗМЕНЕНО
            ...
            conf = probs[pred]
            # ... (остальная часть recognize без изменений) ...
...
if __name__ == '__main__':
    ...
    display_sample_predictions(winner_cppn_genome, config, data_holder, total_images=100, images_per_plot=10)
